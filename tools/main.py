...
def question_one():
    ...
    df = ReadData_robust("data/3-附件1-2025校赛第2轮A题边界层高度与天气数据.csv", ["date", "U向分量风/m/s", "V向分量风/m/s", "2m气温/K",
    ...
    # ==== 共线性分析 ====
    vif_df = vif_get_draw(df.iloc[:,1:-1])
...
def question_two(staff):
    ...
    # ==== 数据读取与预处理，还有训练集划分 ====
    # data = preprocess_and_save('data/3-附件1-2025校赛第2轮A题边界层高度与天气数据.csv',
    #                            'data/4-附件2-2025校赛第2轮A题嘉兴空气质量数据.csv', 'processed_data.csv', staff)
...
def question_three():
    ...
    df = ReadExcel_robust('data/3-附件1-2025校赛第2轮A题边界层高度与天气数据.csv', staff=1)
